...
/// Color scheme for the terminal (foreground / background / caret) — iOS-Variante (UIColor).
struct TerminalTheme: Identifiable, Hashable {
    ...
    static let all: [TerminalTheme] = [black, beam, midnight, solarizedDark, dracula, light]

    static func by(id: String) -> TerminalTheme { all.first { $0.id == id } ?? beam }
}
