...
actor LocalTranscriptionService {
    ...
    /// Baut einen Live-Stream-Transkriber aus dem geladenen WhisperKit-Modell.
    func makeLiveTranscriber(
        modelName: String,
        language: String,
        onState: @escaping AudioStreamTranscriberCallback
    ) async throws -> AudioStreamTranscriber {
        let kit = try await pipeline(modelName: modelName)
        guard let tokenizer = kit.tokenizer else {
            throw LocalTranscriptionError.modelMissing(Self.modelURL(named: modelName))
        }
