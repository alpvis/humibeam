...
final class AppState {
    ...
    /// If set and it returns true, transcribed text was consumed (e.g. typed into the
    /// active terminal session) and the normal auto-paste is skipped.
    var directTextSink: ((String) -> Bool)?
    /// Opens the terminal window from the menu-bar popover (menu bar is the app's home).
    ...
    var activeProfile: PromptProfile {
        appSettings.promptProfiles.first { $0.id == appSettings.selectedProfileID }
            ?? appSettings.promptProfiles.first
            ?? PromptProfile.defaults[0]
    }
    ...
    func startWorkflow(_ type: WorkflowType, source: WorkflowLaunchSource = .manual) {
        ...
        case .localTranscription:
            if appSettings.liveTranscriptionEnabled {
                let workflow = LiveTranscriptionWorkflow(
        ...
        case .textImprover:
            var profileSettings = textImprovementSettings
            profileSettings.systemPrompt = activeProfile.prompt
            let workflow = TextImprovementWorkflow(
                settings: profileSettings,
                language: transcriptionSettings.language,
                model: activeProfile.smart ? .rageMode : .fastEdit
            )
            configureWorkflowHandlers(workflow)
            activeWorkflow = workflow
            workflow.start()
        ...
        if appSettings.soundFeedbackEnabled { SoundService.recordStart() }
    ...
    private func handleWorkflowOutput(_ text: String) {
        if let type = activeWorkflow?.type {
            history.record(text, type: type)
            usage.recordDictation(characters: text.count)
        }
        if appSettings.soundFeedbackEnabled { SoundService.done() }
        // Prefer typing into the active terminal session (humibeam); else paste into frontmost app.
        if directTextSink?(text) != true {
    ...
    private func handleWorkflowPhaseChange(_ phase: WorkflowPhase, workflow: any Workflow) {
        ...
        case .error:
            ...
            if appSettings.soundFeedbackEnabled { SoundService.failure() }
            if activeLaunchSource == .hotkeyBackground {
                activeWorkflow = nil
                activePasteTarget = nil
                page = .main
